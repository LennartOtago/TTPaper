...
\begin{abstract}
	...
	Given some simulated data, this paper presents a hierarchical Bayesian framework to infer pressure, temperature and ozone concentrations values and some other hyper-parameters such as the noise covariance and the ozone smoothness.
...
In doing so, the pressure and temperature profile is parametrise, whereas a non-paremeteric model for the ozone is used.
...
We can see that ozone and pressure are higly correlated.
the pressure is sligkty off
the ozone peak is much lowe
the second ozone peak is not recovered.
\begin{figure}[ht!]
	\centering
...
The results show that a small change of pressure does have a big influence on the ozone profile.
One could extend the current parametrised model of pressure to a non-parametric model, see e.g., \cite{RenateNonPara2026}.
It is clear the data is uninformative about the ozone concentration in higher atmospheric regions and the second ozone peak is not recovered.
...
To obtain a approximation of the marginal posterior \text{xxxx} function evaluations are required.
Once the TT is composed every second sample generated by the SIRT scheme present an independent sample of the marginal posterior.
