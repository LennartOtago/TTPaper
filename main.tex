...
First a $d$-dimensional grid with $n$ grid points in for each dimension in the parameter space has to be defined.
Because that is a parameter space of $n^d$ grid points, which it is computationally not feasible to represent naively in a $d$-dimensional tensor, a TT is employed to approximate the target function on that parameter space.
In practise a TT approximation may have negative or zero values in regions where the true function value is small and positive.
...
For now assume the ranks $r_k = r$ for $k =1, \dots, d-1$, then the TT approximates the function space with $n \times r^2 \times (d-2) + n \times r \times 2$ instead of the $n^d$ function evaluations traditionally required.
...
\begin{align}
		...
		\end{split}
\end{align}
We call each function $\pi( \theta_k | \theta_{k-1},\dots, \theta_1, \bm{y}) \in \mathbb{R}^n$ the 'conditional marginal' beacuse it is conditioned on $\theta_{k-1},\dots, \theta_1, $ and marginalised over $\theta_{k+1} , \dots, \theta_d$.
To obtain these is computationally relatively cheap because each of the TT cores is representative for one dimensions in the parameter space and hence it is easy to integrate over the individual parameters $\theta_k \in \bm{\theta}$ for $k = 1, \dots, d$.
...
Note that, when conditioning on a parameter value the core $\bm{G}^2_k(\theta_k)\in \mathbb{R}^{r_{k-1} \times r_{k}}$.

The 'conditional marginal' defines mapping from the uniform distribution to the target function, which is constructed via the cumulative distribution function (CDF)
\begin{align}
	F(\theta_k) = \int_{-\infty}^{\theta_k} \pi( \hat{\theta}_k | \theta_{k-1},\dots, \theta_1, \bm{y}) \diff \hat{\theta}_k  \, .
	\label{eq:CurrCDF}
\end{align}
Then a uniformly distribute random variable $\bm{u} \sim \mathcal{U} [0,1]^d$ is projected onto the parameter space via the inverse
\begin{align*}
	\theta_k = F^{-1}(u_k)
\end{align*}
with $u_k \in \bm{u}$.
This scheme is repeated for each 'conditional marginal' in Eq. \ref{} until a sample $\bm{\theta} \sim \pi (\bm{\theta}|\bm{y})$ is obtained.

\textcolor{red}{In practise a small constant $\xi$ according to the L2-norm error of the TT approximation is added so that 
\begin{align}
	\pi( \bm{\theta}  | \bm{y}) \approx \xi + \tilde{g}(\bm{\theta})^2
\end{align}
This ensure positivity and makes each of mappings as in Eq. \ref{} well defined.
This is to ensure that one doesn’t get stuck in the correction step.
...
Further we can use that to bound the maximum absolute error $\epsilon$ as in \cite{}.
So the error is $(\tau_{\text{int}} -1 )/2  \leq \epsilon$.
