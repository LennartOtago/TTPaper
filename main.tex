...
The posterior pressure profiles are slightly larger than its ground truth.

\begin{figure}[ht!]
	\centering
	\includegraphics[]{TTPaperFirstPressAffine.png}
	\caption[]{30 posterior pressure profiles from marginal posterior samples based on the approximated model via Eq.~\ref{eq:pressFunc}.}
...
\begin{figure}[ht!]
	...
	\caption[]{30 posterior temperature profiles from marginal posterior samples based on the approximated model via Eq.~\ref{eq:tempFunc}.}
...
That is why if the pressure values are slightly larger than its ground truth, see Fig.~\ref{fig:AffPressSampl}, then the ozone concentration is much lower than its ground truth, see Fig.~\ref{fig:AffOzonSampl}.
