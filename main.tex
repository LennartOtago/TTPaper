...
\usepackage[backend=bibtex,style=numeric,natbib=true,maxbibnames=5,giveninits=true, useprefix]{biblatex}
...
\label{sec:BayModel}
Here a hierarchical Bayesian model is developed where the forward model is described by either $\bm{A}_L(\bm{p},\bm{T})$ the linearised RTE  (see Eq.~\ref{eq:RTE}) or by the approximation $\bm{M} \bm{A}_L(\bm{p},\bm{T})$ to the non-linear RTE, where $\bm{M}$ is an affine map.
...
\begin{align}
	\bm{A}_{\bm{\Phi}} \coloneqq \bm{M} \bm{A}_L(p_0,b,T_0,\bm{h_T},\bm{a}) \, ,
\end{align}
where for now $\bm{M} = \bm{I}$.
Consequently, with the priors and simulated data, the marginal and conditional posterior are well defined and a Tensor-Train approximation of the marginal posterior is approximated using a tensor-train (TT) and the squared inverse Rosenblatt transform (SIRT) \cite{} is employed to generate samples from the marginal posterior.
Conditioned on those samples the randomise then optimise (RTO) method \cite{bardsley2015randomize} is used to sample from the full conditional posterior.

\subsubsection{Tensor-Train Approximation of the Marginal Posterior}

Instead of using conventional Markov-Chain Monte-Carlo methods to generate samples from the marginal posterior, it is approximated directly using a TT representation.
In the following a brief introduction to the TT format is provided.
\cite{cui2022deep}
\cite{OSELEDETS2010TTCross}
\cite{dolgov2020approximation}
\cite{christentwalkaccess}
\cite{alexDeepPy}

Alternatively t-walk \cite{christen2010general}
...
Representing the marginal posterior naively as a $d$-dimensional tensor with $n^d$ grid points is computationally not feasible. 
...
Given a TT approximation, samples from the target function are generated via the inverse Rosenblatt transform (IRT) scheme \cite{}, more precisely, the squared inverse Rosenblatt transform (SIRT) since we approximate the square root of the target function.
...
\begin{align}
	...
	\end{split}
\end{align}
We call each function $\pi( \theta_k | \theta_{k-1},\dots, \theta_1, \bm{y}) \in \mathbb{R}^n$ the 'conditional marginal' because it is conditioned on $\theta_{k-1},\dots, \theta_1, $ and marginalised over $\theta_{k+1} , \dots, \theta_d$.
...
\begin{align}
	...
	\end{split}
\end{align}
where each of the integrals reduce a core $\bm{G}_k \in \mathbb{R}^{r_{k-1}\times n \times r_{k}}$ to $\int \bm{G}_k^2(\theta_k)  \, \diff \theta_k \in \mathbb{R}^{r_{k-1} \times r_{k}}$ and $z$ is a normalisation constant see \cite{}.
...
Note that, for a specific parameter value $\theta_k$ the core $\bm{G}^2_k(\theta_k)\in \mathbb{R}^{r_{k-1} \times r_{k}}$ is given according to the used interpolation basis, see, e.g., \cite{} for piecewise linear interpolation.
...
\end{align*}
with $u_k \in \bm{u}$.
This scheme is repeated for each 'conditional marginal' in Eq. \ref{} until a sample $\bm{\theta} \sim \tilde{\pi} (\bm{\theta}|\bm{y})$ is obtained.

Since the samples $\{ \bm{\theta}^{(1)} , \dots, \bm{\theta}^{(N)}\} \sim \tilde{\pi} (\bm{\theta}|\bm{y})$ are drawn from an approximation they are corrected with an Metropolis--Hastings (MH) algorithm \cite{}.
...
\begin{align}
	h(\bm{\theta}, \bm{\theta}^{\prime}  )  = \min (h(\bm{\theta}, \bm{\theta}^{\prime}  ) ,1) \, .
\end{align}
...
In practise a small constant $\xi$ according to the L2-norm error of the TT approximation is added so that 
\begin{align}
	\pi( \bm{\theta}  | \bm{y}) \approx \tilde{\pi}( \bm{\theta}  | \bm{y}) = \frac{1}{z} (\xi + \tilde{g}(\bm{\theta})^2) \, .
\end{align}
This has the consequence that the chain of the MH correction algorithm is uniform geometric convergent (and ergodic \cite{}) towards $\pi( \bm{\theta}  | \bm{y})$, because
\begin{align}
	 \tilde{w} =\left\lVert	\frac{\pi( \bm{\theta}  | \bm{y})}{\tilde{\pi}( \bm{\theta}  | \bm{y})} \right\rVert_{\infty} = \sup \frac{\pi( \bm{\theta}  | \bm{y})}{\xi + \tilde{g}(\bm{\theta})^2} < \infty
\end{align}
is bounded \cite{}.
Without $\xi$ the TT approximation $\tilde{g}(\bm{\theta})^2$ may be arbitrarily small in regions where the true function is small and the ratio in \ref{} can not be bounded.
With $\xi$ we introduce a more or less uniform error.
Further \cite{} show that if $\xi$ is chosen according to the 2-norm error
\begin{align}
...
Further we calculate the integrated autocorrelation time (IACT) as in \cite{} using the code from \cite{}.
...
As already mentioned we use the \textcolor{red}{TT toolbox}, which uses the TT-Cross amen scheme to approximate the target function.
By exploratory analysis we define ranks $r=$, grid size $n=$ and do one sweep (left-to-right and right-to-left) in the TT-Cross.
On a \textcolor{red}{Laptop} \textcolor{red}{xxxx} function evaluate takes about $1$min.
To sample from the TT approximation we generate 2000 samples from with the SIRT scheme and correct those with an importance sampling step, as in \cite{}.
...
\begin{figure}[ht!]
	...
	\caption[]{29  temperature samples.}
	\label{fig:TempSampl}
\end{figure}
...
To sample an posterior ozone profile from the full conditional $\pi(\bm{x} |\bm{\Phi},\delta, \gamma, \bm{y})$ we use the RTO method, as in~\cite{bardsley2012mcmc, bardsley2014randomize,BardsleyTC2019RTO,fox2016fast}.
...
More explicitly, conditioned on an independent $\bm{\Phi}^{(k)}, \delta^{(k)},\gamma^{(k)} \sim \pi_{\bm{\theta}}(\cdot | \bm{y})$, one independent full conditional posterior sample is given as  
\begin{align}
	\label{eq:RTOAppl}
...
The Cholesky factorisation of $\bm{B}^{(k)}$ and $\bm{L}$ is obtained via the Python function \textcolor{red}{torch} \texttt{numpy.linalg.cholesky} and \textcolor{red}{torch} \texttt{scipy.linalg.cho\_solve} is used to solve for $\bm{x}^{(k)}$.
We draw $n-1 = 29$\textcolor{red}{xxx}  posterior samples in $\approx 0.003$.
...
More specifically $N$ samples $\bm{x}^{(k)} \sim \pi_{\bm{x}}(\cdot|\bm{\Phi}^{(k)}, \delta^{(k)} , \gamma^{(k)},\bm{y})$, for $k = 1, \dots,N$, from the posterior to generate,
\begin{align*}
	\bm{W} = \begin{bmatrix}
		\vert&   &  \vert & & \vert \\
...
\end{align*}
using the Python function \textcolor{red}{\texttt{numpy.linalg.leassqauresolve}}.
...
\begin{figure}[ht!]
	...
	\caption[]{29 pressure samples, approximated model.}
	\label{fig:PressSampl}
\end{figure}
...
\begin{figure}[ht!]
	...
	\caption[]{29  temperature samples, approximated model.}
	\label{fig:TempSampl}
\end{figure}
...
\begin{figure}[ht!]
	...
	\caption[]{29 Ozone samples via RTO method and posterior ozone mean via quadrature (see Eq.~\ref{eq:MeanInt}) with aproximated model.}
	\label{fig:OzonSampl}
\end{figure}
\section{Conclusion}
\begin{itemize}
