...
Given a TT approximation samples from the target function are generated via the inverse Rosenblatt transport (IRT) scheme \cite{}, more precisely the squared inverse Rosenblatt transport (SIRT) since we approximate the square root of the target function.
...
\end{align*}
This is computationally relatively cheap because each of the TT cores is representative for one dimensions in the parameter space and hence it is easy to integrate over the individual parameters $\theta_k \in \bm{\theta}$ for $k = 1, \dots, d$.
...
If you want to use a quadrature scheme to calculate expectation or are only interested in the individual marginals you are done here since other marginals can be computed the same way.
Further within the SIRT each one-dimensional 'conditional marginal' $\pi( \theta_k | \theta_{k-1},\dots, \theta_1, \bm{y})$ conditioned on $\theta_{k-1},\dots, \theta_1, $ and marginalised over $\theta_{k+1} , \dots, \theta_d$.
When conditioning on a parameter value e.~g., $\bm{G}^2_k(\theta_k)\in \mathbb{R}^{r_{k-1} \times r_{k}}$ and the 'conditional marginal' $\pi( \theta_k | \theta_{k-1},\dots, \theta_1, \bm{y}) \in \mathbb{R}^n$.
...
\end{align*}
with $u_k \in \bm{u}$.
This scheme is repeated until a sample $\bm{\theta} \sim \pi (\bm{\theta}|\bm{y})$ is obtained.
