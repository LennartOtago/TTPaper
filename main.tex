...
\begin{abstract}
	...
	Given some simulated data, this paper presents a hierarchical Bayesian framework to infer ozone concentrations as well as the pressure and temperature profile and some other hyper-parameters such as the noise covariance and the ozone smoothness.
...
Further in the current data analysis pressure and temperature values are taken from 3rd party measurements and their uncertainties are not included.

In this paper we develop a model so that pressure and temperature are treated as unknowns and are included within the inversion process.
In doing so, we parametrise the pressure and temperature profile, whereas a non-paremeteric model for the ozone is used.
...
A linear-Gaussian hierarchical Bayesian framework is employed to infer the ozone concentration in the stratosphere as well as provide pressure and temperature values.
This includes establishing a hierarchical structure and classifying hyper-parameters and parameters.
For efficient inference we employ the marginal-then-conditional (MTC) scheme as in~\cite{fox2016fast} and extend it to include model describing hyper-parameter related to pressure and temperature.
...
We show that approximating the marginal posterior on a grid using a functional tensor-train (TT) is an efficient alternative to conventional sampling-based methods to explore the hyper-parameters space.
...
In the following, a parameter and hyper-parameters are classified and a choice of prior distributions is established.
...
Following the MTC scheme, we characterise the marginal posterior over the hyper-parameters $\bm{\theta} = \{ \bm{\Phi}, \gamma, \delta \}$ first and then the full conditional posterior for the parameter $\bm{x}$.
...
Conditioned on those hyper-parameter samples the randomise-then-optimise (RTO) method~\cite{bardsley2015randomize} is used to sample from the full conditional posterior.
...
Instead of using conventional Markov-Chain Monte-Carlo methods, such as, e.g. the t-walk~\cite{christentwalkaccess}, to generate samples from the marginal posterior, it is approximated directly using a TT representation.
...
\end{align}
This has the consequence that the chain of the MH correction algorithm is uniform geometric convergent (and ergodic~\cite{dolgov2020approximation}) towards $\pi( \bm{\theta}  | \bm{y})$, because
\begin{align}
	\label{eq:ErroRatio}
...
Further~\cite{cui2022deep} show that if $\xi$ is chosen according to the L2-norm error
\begin{align}
...
To generate a TT for $\sqrt{\pi( \bm{\theta}  | \bm{y})}$ we use the \texttt{deep-tensor-py} package~\cite{alexDeepPy}, which implements the TT-cross algorithm~\cite{OSELEDETS2010TTCross} to generate a TT approximation.
Further, A. de Beer~\cite{alexDeepPy} has implemented the SIRT scheme from~\cite{cui2022deep}.
By exploratory analysis we define ranks $r=10$ manually and set the grid size $n=30$ for the 18-dimensional parameter space, see Tab.\ref{tab:priors} for the grid bounds.
Then one sweep (left-to-right and right-to-left) in the TT-cross with first order Legendre polynomials as basis functions provides an accurate enough function approximation of the marginal posterior.
This requires \textcolor{red}{$xx$} function evaluations.
Note that we arranged the hyper-parameter space so that highly correlated hyper-parameters are adjacent.
...
According to the IACTs, see Tab.~\ref{tab:priors}, every second sample is an independent sample from the marginal posterior.
Once the TT is formed 2 function evaluations per independent sample are required.
...
\begin{figure}[ht!]
	...
	\caption[]{30 ozone samples from the full conditional posterior via the RTO method.}
...
The independent marginal posterior samples give posterior pressure and temperature profiles through their respective functions, see Eq.~\ref{eq:pressFunc} and Eq.~\ref{eq:tempFunc}.
...
\begin{figure}[ht!]
	...
	\caption[]{30 posterior pressure samples based on the approximated model.}
...
\begin{figure}[ht!]
	...
	\caption[]{29 posterior temperature samples based on the approximated model.}
...
\begin{figure}[ht!]
	...
	\caption[]{30 posterior ozone samples via the RTO method based on the approximated model.}
	\label{fig:AffOzonSampl}
\end{figure}

\section{Conclusion}

We report a IACT of close to one so that every second sample is an independent sample.. that is close to optimal.
We show that ozone and pressure are highly correlated
2nd ozone peak is not included.
Once the TT is formed 2 function evaluations per independent sample are required.
more loos unparameterise moedl for pressure see Angela 
\clearpage
